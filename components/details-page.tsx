...
export default function DetailsPage({ defaultType = "income" }: DetailsPageProps) {
  ...
  useEffect(() => {
    if (dateRange.from) {
      setFilters((prev) => {
        ...
        const startDateChanged = prev.startDate?.getTime() !== dateRange.from.getTime()
        const endDateChanged = prev.endDate?.getTime() !== (dateRange.to || dateRange.from).getTime()
        ...
        return prev // Retorna o estado anterior se não houver mudanças
      })
    }
  }, [dateRange])
  ...
  const applyFilters = useCallback(() => {
    setFilters((prev) => {
      // Verificar se os valores realmente mudaram antes de atualizar
      const categoriesChanged = JSON.stringify(prev.categories) !== JSON.stringify(selectedCategories)
      ...
      <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-6">
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              <Label>Categorias de {activeType === "income" ? "Entrada" : "Saída"}</Label>
